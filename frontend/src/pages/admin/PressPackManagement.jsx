...
// Press Pack Management Form Modal Component
const PressPackManagementFormModal = ({ isOpen, onClose, record, onSave }) => {
  ...
  useEffect(() => {
    if (record) {
      setFormData({
        distribution_package: record.distribution_package || '',
        ...
        no_of_indexed_websites: record.no_of_indexed_websites || '',
        no_of_non_indexed_websites: record.no_of_non_indexed_websites || '',
        image: record.image || '',
        link: record.link || '',
        words_limit: record.words_limit || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.set('no_of_indexed_websites', parseInt(formData.no_of_indexed_websites) || 0);
      formDataToSend.set('no_of_non_indexed_websites', parseInt(formData.no_of_non_indexed_websites) || 0);
      formDataToSend.set('words_limit', parseInt(formData.words_limit) || 0);
      ...
      if (record) {
        await api.put(`/admin/press-packs/${record.id}`, formDataToSend, config);
      } else {
        await api.post('/admin/press-packs', formDataToSend, config);
      }
      ...
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Indexed Websites</label>
              <input
                type="number"
                min="0"
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Non-Indexed Websites</label>
              <input
                type="number"
                min="0"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px', marginTop: '16px' }}>
            <div style={formGroupStyle}>
              <label style={labelStyle}>News</label>
              <textarea
                value={formData.news}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '32px', gap: '12px' }}>
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Saving...' : (record ? 'Update Press Pack' : 'Create Press Pack')}
            </button>
          </div>
        </form>
      </div>
...
const PressPackManagementPage = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await api.get(`/admin/press-packs?${params.toString()}`);
      setRecords(response.data.pressPacks || []);
  ...
  const handleDeleteRecord = async (recordId) => {
    ...
    try {
      setLoading(true);
      await api.delete(`/admin/press-packs/${recordId}`);
      fetchRecords();
            ...
            <main style={{ flex: 1, minWidth: 0 }}>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)', overflow: 'hidden' }}>
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  Loading press pack management records...
                </div>
              </div>
            </main>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
              <div style={{ background: '#fff', borderRadius: 12, padding: 18, boxShadow: '0 8px 20px rgba(2,6,23,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#757575' }}>Total Press Packs</div>
                </div>
              </div>
            </div>
                      ...
                      <tr key={record.id} style={{
                        ...
                        <td style={{ padding: '16px' }}>
                          <span style={{
                            ...
                            color: record.is_active ? '#166534' : '#dc2626',
                            borderRadius: '12px',
                            fontSize: '11px',
                            fontWeight: '600',
              ...
              {records.length === 0 && (
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  ...
                  <div style={{ fontSize: '16px' }}>
                    Try adjusting your search criteria or add a new press pack.
                  </div>
                </div>
              )}
            </div>
...
export default PressPackManagementPage;