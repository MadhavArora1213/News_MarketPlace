...
const AwardCreationFormModal = ({ isOpen, onClose, record, onSave }) => {
  ...
  useEffect(() => {
    if (record) {
      setFormData({
        ...
        award_organiser_name: record.award_organiser_name || '',
        url: record.url || '',
        tentative_month: record.tentative_month || '',
        industry: record.industry || '',
        regional_focused: record.regional_focused || '',
        award_country: record.award_country || '',
        award_city: record.award_city || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add text fields
      submitData.append('award_name', formData.award_name);
      submitData.append('award_organiser_name', formData.award_organiser_name);
      ...
      submitData.append('award_country', formData.award_country);
      submitData.append('award_city', formData.award_city);
      ...
      if (record) {
        await api.put(`/admin/award-creations/${record.id}`, submitData, config);
      } else {
        await api.post('/admin/award-creations', submitData, config);
      }
      ...
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ margin: 0, fontSize: '24px', fontWeight: '800' }}>
            {record ? 'Edit Award Creation' : 'Create Award Creation'}
          </h2>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Award Organiser Name *</label>
              <input
                type="text"
                value={formData.award_organiser_name}
                onChange={(e) => setFormData({ ...formData, award_organiser_name: e.target.value })}
                style={inputStyle}
                required
              />
            </div>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Award Country</label>
              <input
                type="text"
                value={formData.award_country}
                onChange={(e) => setFormData({ ...formData, award_country: e.target.value })}
                style={inputStyle}
              />
            </div>

            <div style={formGroupStyle}>
              <label style={labelStyle}>Award City</label>
              <input
                type="text"
                value={formData.award_city}
                onChange={(e) => setFormData({ ...formData, award_city: e.target.value })}
                style={inputStyle}
              />
            </div>
          ...
          <div style={{ marginTop: '16px', padding: '20px', border: '2px dashed #e0e0e0', borderRadius: '8px', backgroundColor: '#fafafa' }}>
            <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: '600', color: '#212121' }}>Award Image</h3>
...
const AwardCreationPage = () => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to access award creation management.</p>
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await api.get(`/admin/award-creations?${params.toString()}`);
      setRecords(response.data.awardCreations || []);
  ...
  const handleDeleteRecord = async (recordId) => {
    ...
    try {
      setLoading(true);
      await api.delete(`/admin/award-creations/${recordId}`);
      fetchRecords();
            ...
            <main style={{ flex: 1, minWidth: 0 }}>
              <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)', overflow: 'hidden' }}>
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  Loading award creation records...
                </div>
              </div>
            </main>
          </div>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: '16px', marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Award Creation</h1>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
              <div style={{ background: '#fff', borderRadius: 12, padding: 18, boxShadow: '0 8px 20px rgba(2,6,23,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: 10, background: '#e6f0ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon name="trophy" size="lg" style={{ color: '#1976D2' }} />
                </div>
                <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                ...
                <div>
                  ...
                  >
                    ...
                    {[...new Set(records.map(r => r.award_country).filter(Boolean))].map(country => (
              ...
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f8fafc' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '14px', fontWeight: '600', color: theme.textPrimary }}>
                    Award Creation Records ({totalRecords})
                  </span>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  <thead>
                    <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                      {[
                        ...
                        { key: 'award_organiser_name', label: 'Organiser' },
                        { key: 'url', label: 'URL' },
                        { key: 'tentative_month', label: 'Month' },
                        { key: 'industry', label: 'Industry' },
                        { key: 'regional_focused', label: 'Regional' },
                        { key: 'award_country', label: 'Country' },
                        { key: 'award_city', label: 'City' },
...
export default AwardCreationPage;