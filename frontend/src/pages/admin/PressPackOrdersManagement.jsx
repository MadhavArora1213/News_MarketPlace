...
import {
  Search, Eye, Edit, CheckCircle, XCircle, Clock,
...
const PressPackOrdersManagement = () => {
  const { isAuthenticated, hasAnyRole } = useAdminAuth();
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await api.get(`/press-pack-orders?${params.toString()}`);
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const response = await api.put(`/press-pack-orders/${selectedOrder.id}`, updateFormData);
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold" style={{ color: theme.textPrimary }}>
                Press Pack Orders Management
              </h1>
              <p className="mt-2" style={{ color: theme.textSecondary }}>
                Manage and process press pack orders
              </p>
            </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm" style={{ color: theme.textPrimary }}>
                        {order.package_selection || 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm" style={{ color: theme.textPrimary }}>
                        <div>{order.whatsapp_number || 'N/A'}</div>
                        {order.calling_number && <div>{order.calling_number}</div>}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
            ...
            <div style={{
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.name}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Email</label>
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.email}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>WhatsApp Number</label>
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.whatsapp_number || 'Not provided'}</p>
                    </div>
                    {selectedOrder.calling_number && (
                      <div>
                        <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Calling Number</label>
                        <p style={{ color: theme.textPrimary }}>{selectedOrder.calling_number}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Package Selection</label>
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.package_selection || 'Not specified'}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Submitted By Type</label>
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.submitted_by_type || 'Not specified'}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Press Release Selection</label>
                      <p style={{ color: theme.textPrimary }}>{selectedOrder.press_release_selection || 'Not specified'}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Documents */}
              {(selectedOrder.company_registration_document || selectedOrder.letter_of_authorisation || selectedOrder.image || selectedOrder.word_pdf_document) && (
                <div className="mt-6">
                  <h3 className="text-lg font-semibold mb-4" style={{ color: theme.textPrimary }}>
                    Documents
                  </h3>
                  <div className="space-y-3">
                    {selectedOrder.company_registration_document && (
                      <div>
                        <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Company Registration Document</label>
                        <p style={{ color: theme.textPrimary }}>{selectedOrder.company_registration_document}</p>
                      </div>
                    )}
                    {selectedOrder.letter_of_authorisation && (
                      <div>
                        <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Letter of Authorisation</label>
                        <p style={{ color: theme.textPrimary }}>{selectedOrder.letter_of_authorisation}</p>
                      </div>
                    )}
                    {selectedOrder.image && (
                      <div>
                        <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Image</label>
                        <p style={{ color: theme.textPrimary }}>{selectedOrder.image}</p>
                      </div>
                    )}
                    {selectedOrder.word_pdf_document && (
                      <div>
                        <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Word/PDF Document</label>
                        <p style={{ color: theme.textPrimary }}>{selectedOrder.word_pdf_document}</p>
                      </div>
                    )}
                  </div>
                </div>
...
export default PressPackOrdersManagement;