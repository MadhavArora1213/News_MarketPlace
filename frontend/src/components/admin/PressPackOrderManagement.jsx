...
const PressPackOrderManagement = () => {
  ...
  const handleAcceptOrder = async (orderId) => {
    ...
    try {
      await api.put(`/press-pack-orders/${orderId}/accept`);
      fetchOrders();
    } catch (error) {
  ...
  const handleRejectOrder = async (orderId) => {
    ...
    try {
      await api.put(`/press-pack-orders/${orderId}/reject`, { admin_notes: reason });
      fetchOrders();
    } catch (error) {
  ...
  const handleCompleteOrder = async (orderId) => {
    if (!window.confirm('Are you sure you want to mark this press pack order as completed?')) return;

    try {
      await api.put(`/press-pack-orders/${orderId}/complete`);
      fetchOrders();
    } catch (error) {
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)' }}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search orders by press pack name, customer name, or email..."
                    value={searchTerm}
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                            ...
                            <div>
                              <div style={{ fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                                {debouncedSearchTerm || statusFilter ? 'No orders found' : 'No press pack orders available'}
                              </div>
                              <div style={{ fontSize: '16px' }}>
                                {debouncedSearchTerm || statusFilter ? (
                                  ...
                                  'Orders will appear here when users book press pack distributions.'
                                )}
                              </div>
                            </div>
                          </div>
