...
const AwardFormModal = ({ isOpen, onClose, award, onSave }) => {
  ...
  useEffect(() => {
    if (award) {
      setFormData({
        ...
        cta_text: award.cta_text || '',
        description: award.description || '',
        chief_guest: award.chief_guest || '',
        celebrity_guest: award.celebrity_guest || ''
      });
    } else {
      setFormData({
        award_name: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (award) {
        await api.put(`/awards/${award.id}`, formData);
      } else {
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.message || 'Error saving award entry. Please try again.';
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
      ...
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Award Focus</label>
              <input
                type="text"
                value={formData.award_focus}
                onChange={(e) => setFormData({ ...formData, award_focus: e.target.value })}
                style={inputStyle}
              />
            </div>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Award Month</label>
              <select
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Chief Guest</label>
              <input
                type="text"
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Celebrity Guest</label>
              <input
                type="text"
                value={formData.celebrity_guest}
                onChange={(e) => setFormData({ ...formData, celebrity_guest: e.target.value })}
                style={inputStyle}
              />
            </div>
...
const AwardManagement = () => {
  ...
  // Check if user has permission to manage awards
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to access award management.</p>
  ...
  const handleSelectAll = () => {
    if (selectedAwards.length === paginatedAwards.length) {
      setSelectedAwards([]);
    } else {
      setSelectedAwards(paginatedAwards.map(a => a.id));
    }
  };

  const handleBulkDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete ${selectedAwards.length} awards?`)) return;
    ...
    try {
      ...
      alert('Error deleting awards. Please try again.');
    }
  };
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          <main style={{ flex: 1, minWidth: 0, paddingLeft: !isMobile ? leftGap : 0 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Award Management</h1>
                </div>
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)' }}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search by award name or organiser..."
                    value={searchTerm}
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <span style={{ fontSize: '14px', fontWeight: '600', color: theme.textPrimary }}>
                      {selectedAwards.length > 0 ? `${selectedAwards.length} selected` : 'Select awards'}
                    </span>
                    {selectedAwards.length > 0 && (
                      <button
                        onClick={() => setSelectedAwards([])}
                        style={{
                          padding: '4px 8px',
              ...
              <div style={{ overflowX: 'auto', maxHeight: paginatedAwards.length > 50 ? '600px' : 'auto', overflowY: paginatedAwards.length > 50 ? 'auto' : 'visible' }}>
                  ...
                  <thead>
                    <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700', fontSize: '12px', color: theme.textPrimary, textTransform: 'uppercase', letterSpacing: '0.5px', width: '50px' }}>
                        <input
                          type="checkbox"
                          checked={paginatedAwards.length > 0 && selectedAwards.length === paginatedAwards.length}
                      ...
                      <th
                        ...
                        onClick={() => handleSort('award_name')}
                      >
                        Award Name {getSortIcon('award_name')}
                      </th>
                      ...
                      <th
                        style={{ padding: '16px', textAlign: 'left', fontWeight: '700', fontSize: '12px', color: theme.textPrimary, textTransform: 'uppercase', letterSpacing: '0.5px', cursor: 'pointer' }}
                        onClick={() => handleSort('award_month')}
                      >
                        Month {getSortIcon('award_month')}
                      </th>
                      <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700', fontSize: '12px', color: theme.textPrimary, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                        Actions
                      </th>
                      ...
                      >
                        <td style={{ padding: '16px' }}>
                          <input
                            ...
                            onChange={() => handleSelectAward(award.id)}
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: '12px', color: theme.primary, fontWeight: '500' }}>
                                {award.website && (
                                  <a href={award.website} target="_blank" rel="noopener noreferrer" style={{ color: theme.primary, textDecoration: 'none' }}>
                                    🌐 Website
                                  </a>
                                )}
                              </div>
              ...
              {totalPages > 1 && (
                <div style={{ padding: '16px 20px', borderTop: '1px solid #e5e7eb', backgroundColor: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontSize: '14px', color: theme.textSecondary }}>
                    Page {currentPage} of {totalPages} ({sortedAwards.length} total awards)
                  </div>
              ...
              {paginatedAwards.length === 0 && (
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                    {debouncedSearchTerm || focusFilter || monthFilter ? 'No awards found' : 'No awards available'}
                  </div>
