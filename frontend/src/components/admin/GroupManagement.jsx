...
const GroupFormModal = ({ isOpen, onClose, group, onSave }) => {
  ...
  useEffect(() => {
    if (group) {
      setFormData({
        ...
        group_linkedin: group.group_linkedin || '',
        group_instagram: group.group_instagram || ''
      });
    } else {
      setFormData({
        group_sn: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (group) {
        await api.put(`/groups/admin/${group.id}`, formData);
      } else {
        await api.post('/groups/admin', formData);
      }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
            <div style={formGroupStyle}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, group_sn: e.target.value })}
                style={inputStyle}
                required
              />
            </div>
            ...
            <div style={formGroupStyle}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, group_location: e.target.value })}
                style={inputStyle}
                required
              />
            </div>
            ...
            <div style={formGroupStyle}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, group_website: e.target.value })}
                style={inputStyle}
                required
              />
            </div>
            ...
            <div style={formGroupStyle}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, group_linkedin: e.target.value })}
                style={inputStyle}
              />
            </div>
            ...
            <div style={formGroupStyle}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, group_instagram: e.target.value })}
                style={inputStyle}
              />
            </div>
          </div>
...
const GroupManagement = () => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to access group management.</p>
  ...
  const handleDeleteGroup = async (groupId) => {
    ...
    try {
      await api.delete(`/groups/admin/${groupId}`);
      fetchGroups();
    } catch (error) {
  ...
  const handleStatusChange = async (groupId, newStatus) => {
    try {
      await api.put(`/groups/admin/${groupId}`, { status: newStatus });
      fetchGroups();
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete ${selectedGroups.length} groups?`)) return;
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          <main style={{ flex: 1, minWidth: 0, paddingLeft: !isMobile ? leftGap : 0 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Group Management</h1>
                </div>
                <p style={{ marginTop: 8, color: '#757575' }}>Manage news publication groups</p>
              </div>
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)' }}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search groups by name, SN, or location..."
                    value={searchTerm}
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const headers = ['Name', 'SN', 'Location', 'Website', 'LinkedIn', 'Instagram', 'Status'];
                        const csvData = [
                          headers.join(','),
                          ...filteredGroups.map(group => [
                            ...
                            `"${group.group_website}"`,
                            `"${group.group_linkedin || ''}"`,
                            `"${group.group_instagram || ''}"`,
                            group.status
                          ].join(','))
                        ].join('\n');
              ...
              <div style={{ overflowX: 'auto', maxHeight: paginatedGroups.length > 50 ? '600px' : 'auto', overflowY: paginatedGroups.length > 50 ? 'auto' : 'visible' }}>
                  ...
                  <thead>
                    <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '16px', textAlign: 'left', fontWeight: '700', fontSize: '12px', color: theme.textPrimary, textTransform: 'uppercase', letterSpacing: '0.5px', width: '50px' }}>
                        <input
                          type="checkbox"
                          checked={paginatedGroups.length > 0 && selectedGroups.length === paginatedGroups.length}
                  ...
                  <tbody>
                    {paginatedGroups.map((group, index) => (
                      ...
                      >
                        ...
                        <td style={{ padding: '16px' }}>
                          <div>
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                              ...
                              {group.group_linkedin && (
                                <a
                                  href={group.group_linkedin}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  style={{ color: '#0077b5', textDecoration: 'none', fontSize: '12px' }}
                                >
                                  💼 LinkedIn
                                </a>
                              )}
                              {group.group_instagram && (
                                <a
                                  href={group.group_instagram}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  style={{ color: '#e4405f', textDecoration: 'none', fontSize: '12px' }}
                                >
                                  📷 Instagram
                                </a>
                              )}
                        ...
                        <td style={{ padding: '16px' }}>
                          <span style={getStatusStyle(group.status)}>
                            {statusOptions.find(opt => opt.value === group.status)?.label || group.status}
                          </span>
                        </td>
                        <td style={{ padding: '16px' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            <button
                              onClick={() => handleStatusChange(group.id, group.status === 'approved' ? 'pending' : 'approved')}
                              style={{
                                padding: '6px 10px',
                                backgroundColor: group.status === 'approved' ? '#ff9800' : '#4caf50',
                                color: '#fff',
                                border: 'none',
                                borderRadius: '4px',
                                fontSize: '12px',
                              ...
                              disabled={!hasAnyRole(['super_admin', 'content_manager'])}
                              onMouseEnter={(e) => e.target.style.backgroundColor = group.status === 'approved' ? '#f57c00' : '#388e3c'}
                              onMouseLeave={(e) => e.target.style.backgroundColor = group.status === 'approved' ? '#ff9800' : '#4caf50'}
                            >
                              {group.status === 'approved' ? 'Deactivate' : 'Approve'}
                            </button>
                            <button
              ...
              <div style={{ padding: '16px 20px', borderTop: '1px solid #e5e7eb', backgroundColor: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ fontSize: '14px', color: theme.textSecondary }}>
                  Showing {groups.length > 0 ? ((currentPage - 1) * pageSize) + 1 : 0}-{Math.min(currentPage * pageSize, totalGroups)} of {totalGroups} groups
                </div>
              ...
              {paginatedGroups.length === 0 && !loading && (
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                    {debouncedSearchTerm ? 'No groups found' : 'No groups available'}
                  </div>
                  <div style={{ fontSize: '16px', marginBottom: '16px' }}>
                    {debouncedSearchTerm ? (
                      <>
                        No groups match your search for "<strong>{debouncedSearchTerm}</strong>".
                        <br />
