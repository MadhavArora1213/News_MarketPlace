...
const ThemeOrderManagement = () => {
  const { admin, logout, hasRole, hasAnyRole } = useAdminAuth();

  // Check if user has permission to manage theme orders
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: theme.backgroundSoft }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: theme.textSecondary }}>You don't have permission to access theme order management.</p>
  ...
  const statusOptions = [
    ...
    { value: 'confirmed', label: 'Confirmed', color: theme.info },
    { value: 'completed', label: 'Completed', color: theme.success },
    { value: 'cancelled', label: 'Cancelled', color: theme.danger }
  ];
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await api.get(`/theme-orders?${params}`);
      ...
      console.error('Error fetching theme orders:', error);
      if (error.response?.status === 401) {
        localStorage.removeItem('adminAccessToken');
        window.location.href = '/admin/login';
        return;
      }
      alert('Failed to load theme orders. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      const response = await api.get(`/theme-orders/export-csv?${params.toString()}`, {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'theme_orders.csv');
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await api.put(`/theme-orders/${orderId}`, { status: newStatus });
      fetchOrders();
    } catch (error) {
  ...
  const handleDeleteOrder = async (orderId) => {
    if (!window.confirm('Are you sure you want to delete this theme order?')) return;

    try {
      await api.delete(`/theme-orders/${orderId}`);
      fetchOrders();
    } catch (error) {
          ...
          <main style={{ flex: 1, minWidth: 0, paddingLeft: !isMobile ? leftGap : 0 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, border: `4px solid #000`, display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 24, alignItems: 'center' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <h1 style={{ margin: 0, fontSize: 34, fontWeight: 800 }}>Theme Order Management</h1>
                </div>
                <p style={{ marginTop: 8, color: '#757575' }}>Manage theme collaboration orders and requests</p>
              </div>
              <button
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)' }}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search by theme name, customer name, or email..."
                    value={searchTerm}
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 8px 20px rgba(2,6,23,0.06)', overflow: 'hidden' }}>
                  ...
                  <tbody>
                    {orders.map((order, index) => (
                      ...
                      >
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div style={{
                              ...
                              <span style={{ fontSize: '16px', color: theme.textSecondary, fontWeight: '600' }}>
                                {order.customer_info?.fullName?.charAt(0).toUpperCase() || 'U'}
                              </span>
                            </div>
                            <div>
                        ...
                        <td style={{ padding: '16px' }}>
                          <span className="text-sm" style={{ color: theme.textPrimary, fontWeight: '600' }}>
              ...
              <div style={{ padding: '16px 20px', borderTop: '1px solid #e5e7eb', backgroundColor: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ fontSize: '14px', color: theme.textSecondary }}>
                  Showing {totalCount > 0 ? ((currentPage - 1) * pageSize) + 1 : 0}-{Math.min(currentPage * pageSize, totalCount)} of {totalCount} orders
                </div>
              ...
              {orders.length === 0 && (
                <div style={{ padding: '80px', textAlign: 'center', color: theme.textSecondary }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                    {searchTerm || statusFilter ? 'No orders found' : 'No theme orders available'}
                  </div>
                  <div style={{ fontSize: '16px', marginBottom: '16px' }}>
                    {searchTerm || statusFilter ? (
                      ...
                      'Theme orders will appear here when users place collaboration requests.'
                    )}
                  </div>
...
export default ThemeOrderManagement;