...
// Group publications by region for creating groups
const groupPublicationsByRegion = () => {
  const grouped = {};
...
async function findOrCreateGroup(groupName, groupWebsite = '', groupLinkedin = '', groupInstagram = '') {
  ...
  try {
    ...
    const groupData = {
      ...
      group_location: 'UAE/Middle East', // Default location
      submitted_by: null,
...
// Helper function to find publication by name
async function findPublicationByName(name) {
  try {
...
async function processPublication(pubData) {
  try {
    // Check if publication already exists by name
    const existing = await findPublicationByName(pubData.publicationName);
    
    if (existing) {
      console.log(`Publication ${pubData.publicationName} already exists, skipping...`);
      return false;
    }

    // Create or find group for this publication's region
    const groupId = await findOrCreateGroup(`${pubData.region} Publications`);
    
    // Parse numeric fields
    const da = pubData.da ? parseInt(pubData.da) : null;
    const dr = pubData.dr ? parseInt(pubData.dr) : null;
    const agreementTat = pubData.committedDaysTat ? parseInt(pubData.committedDaysTat) : null;
    ...
    // Parse do_follow_link
    const doFollow = pubData.doFollow && pubData.doFollow.toLowerCase().includes('do follow');

    // Map data to Publication model fields
    const publicationDataForModel = {
      group_id: groupId,
      publication_sn: `PUB-${pubData.sn.padStart(3, '0')}`,
      publication_grade: 'A', // Default grade
      publication_name: pubData.publicationName,
      publication_website: pubData.publicationUrl || null,
      publication_price: priceUsd,
      agreement_tat: agreementTat,
      practical_tat: practicalTat,
      publication_socials_icons: JSON.stringify([]), // Empty for now
      publication_language: pubData.language || null,
      publication_region: pubData.region || null,
      publication_primary_industry: pubData.publicationPrimaryFocus || null,
      website_news_index: null, // Not available in data
      da: da,
      dr: dr,
      sponsored_or_not: false,
      words_limit: 1000, // Default word limit
      number_of_images: 3, // Default number of images
      do_follow_link: doFollow,
      example_link: pubData.articleReferenceLink || null,
      excluding_categories: null,
      other_remarks: pubData.remarks || null,
      tags_badges: JSON.stringify([]), // Empty for now
    ...
    const savedPublication = await Publication.create(publicationDataForModel);
    console.log(`Saved publication: ${savedPublication.publication_name}`);
    return true;

  } catch (error) {
    console.error(`Error processing publication ${pubData.publicationName}:`, error.message);
    return false;
  }
}

async function populatePublicationData() {
  try {
    console.log('Starting publication data population...');
    ...
    // Create groups first
    console.log('\n=== Creating Groups ===');
    const groupedPublications = groupPublicationsByRegion();
    for (const region of Object.keys(groupedPublications)) {
      await findOrCreateGroup(`${region} Publications`);
    }
    
    console.log('\n=== Processing Publications ===');
    console.log(`Processing ${publicationData.length} publications...`);

    for (const publication of publicationData) {
      const success = await processPublication(publication);
      if (success) {
        totalSuccess++;
      } else {
        totalError++;
    ...
    // Get final counts
    try {
      const totalResult = await db.query('SELECT COUNT(*) as count FROM publications');
      const groupResult = await db.query('SELECT COUNT(*) as count FROM groups');

      console.log(`Total publications in database: ${totalResult.rows[0].count}`);
      console.log(`Total groups in database: ${groupResult.rows[0].count}`);
    } catch (countError) {
      console.log('Could not get final counts:', countError.message);
    }

  } catch (error) {
    console.error('Error in populatePublicationData:', error);
  }
}
...
if (require.main === module) {
  ...
  populatePublicationData()
    .then(() => {
      console.log('\nPublication data population script completed successfully');
      process.exit(0);
    })
    .catch((error) => {
...
module.exports = { populatePublicationData };